...
class TestScanstring:
    def test_scanstring(self):
        scanstring = self.kim_edn.decoder.py_scanstring
        ...
        self.assertEqual(
            scanstring(
                '"A KIMEDN payload should be an object or array, not a string."', 1, True),
            ('A KIMEDN payload should be an object or array, not a string.', 62))
    ...
    def test_bad_escapes(self):
        ...
        for s in bad_escapes:
            with self.assertRaises(self.KIMEDNDecodeError, msg=s):
                scanstring(s, 1, True)

    def test_overflow(self):
        import sys
        with self.assertRaises(OverflowError):
            self.kim_edn.decoder.py_scanstring(b"xxx", sys.maxsize + 1)
