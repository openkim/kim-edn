...
class TestUnicode:
    ...
    def test_bytes_decode(self):
        ...
        # Allow documents that consist of only a string, which can present
        # a special case not covered by the encoding detection patterns
        # specified in RFC-4627 for utf-16-le (XX 00 XX 00).
        self.assertEqual(self.loads('"\u2600"'.encode('utf-16-le')),
                         '\u2600')

        # Encoding detection for small (<4) bytes objects is implemented as
        # a special case. RFC-7159 and ECMA-404 allow single codepoint
        # documents which are only two bytes in utf-16 encodings w/o BOM.
